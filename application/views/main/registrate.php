<main>
    <section id="registrate-main">
        <div id="container">
            <h1>REGÍSTRATE</h1>
            <h3><span>INCIO</span> REGÍSTRATE</h3>
        </div>
    </section>
    <section id="registrate-type">
        <div class="wrap-form">
            <h1>Elija el tipo de usuario para registrase</h1>
            <button id="indv" class="lean-btn registrate-btn">Como Individual</button>
            <button id="fundacion" class="lean-btn registrate-btn">Como Negocio o Fundacion</button>
            <button id="agente" class="lean-btn registrate-btn">Como agente LEAN</button> 
        </div>
    </section>
    <section id="social-contact">
        <div id="container">
            <h4>LEAN EN LAS REDES SOCIALES</h4>
            <div id="social-icon">
            <svg aria-hidden="true" focusable="false" data-prefix="fab" data-icon="twitter" class="svg-inline--fa fa-twitter fa-w-16" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path fill="currentColor" d="M459.37 151.716c.325 4.548.325 9.097.325 13.645 0 138.72-105.583 298.558-298.558 298.558-59.452 0-114.68-17.219-161.137-47.106 8.447.974 16.568 1.299 25.34 1.299 49.055 0 94.213-16.568 130.274-44.832-46.132-.975-84.792-31.188-98.112-72.772 6.498.974 12.995 1.624 19.818 1.624 9.421 0 18.843-1.3 27.614-3.573-48.081-9.747-84.143-51.98-84.143-102.985v-1.299c13.969 7.797 30.214 12.67 47.431 13.319-28.264-18.843-46.781-51.005-46.781-87.391 0-19.492 5.197-37.36 14.294-52.954 51.655 63.675 129.3 105.258 216.365 109.807-1.624-7.797-2.599-15.918-2.599-24.04 0-57.828 46.782-104.934 104.934-104.934 30.213 0 57.502 12.67 76.67 33.137 23.715-4.548 46.456-13.32 66.599-25.34-7.798 24.366-24.366 44.833-46.132 57.827 21.117-2.273 41.584-8.122 60.426-16.243-14.292 20.791-32.161 39.308-52.628 54.253z"></path></svg>
            <svg aria-hidden="true" focusable="false" data-prefix="fab" data-icon="facebook-f" class="svg-inline--fa fa-facebook-f fa-w-9" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path fill="currentColor" d="M215.8 85H264V3.6C255.7 2.5 227.1 0 193.8 0 124.3 0 76.7 42.4 76.7 120.3V192H0v91h76.7v229h94V283h73.6l11.7-91h-85.3v-62.7c0-26.3 7.3-44.3 45.1-44.3z"></path></svg>
            <svg aria-hidden="true" focusable="false" data-prefix="fab" data-icon="instagram" class="svg-inline--fa fa-instagram fa-w-14" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path fill="currentColor" d="M224.1 141c-63.6 0-114.9 51.3-114.9 114.9s51.3 114.9 114.9 114.9S339 319.5 339 255.9 287.7 141 224.1 141zm0 189.6c-41.1 0-74.7-33.5-74.7-74.7s33.5-74.7 74.7-74.7 74.7 33.5 74.7 74.7-33.6 74.7-74.7 74.7zm146.4-194.3c0 14.9-12 26.8-26.8 26.8-14.9 0-26.8-12-26.8-26.8s12-26.8 26.8-26.8 26.8 12 26.8 26.8zm76.1 27.2c-1.7-35.9-9.9-67.7-36.2-93.9-26.2-26.2-58-34.4-93.9-36.2-37-2.1-147.9-2.1-184.9 0-35.8 1.7-67.6 9.9-93.9 36.1s-34.4 58-36.2 93.9c-2.1 37-2.1 147.9 0 184.9 1.7 35.9 9.9 67.7 36.2 93.9s58 34.4 93.9 36.2c37 2.1 147.9 2.1 184.9 0 35.9-1.7 67.7-9.9 93.9-36.2 26.2-26.2 34.4-58 36.2-93.9 2.1-37 2.1-147.8 0-184.8zM398.8 388c-7.8 19.6-22.9 34.7-42.6 42.6-29.5 11.7-99.5 9-132.1 9s-102.7 2.6-132.1-9c-19.6-7.8-34.7-22.9-42.6-42.6-11.7-29.5-9-99.5-9-132.1s-2.6-102.7 9-132.1c7.8-19.6 22.9-34.7 42.6-42.6 29.5-11.7 99.5-9 132.1-9s102.7-2.6 132.1 9c19.6 7.8 34.7 22.9 42.6 42.6 11.7 29.5 9 99.5 9 132.1s2.7 102.7-9 132.1z"></path></svg>
            </div>
        </div>
    </section>
</main>

<section id="registrate-form">
    <div class="modal-content">
        <form name="registration" action="<?php echo current_url(); ?>/validate" onsubmit="return validateForm()" method="POST">  
            <div class="modal-header">
                <h4 id="form-title"></h4>
            </div> 
            <div class="modal-body">
                <input type="hidden" name="form_type" value="" id="form_type">
                <div class="halfleft-textbox">
                <label for="correo">Correo</label>
                <input type="text" id="correo" name="correo" placeholder="Correo" required>
                </div>    
                <div class="halfright-textbox">
                <label for="contrasena">Contraseña</label>
                <input type="password" id="contrasena" name="contrasena" placeholder="Contraseña" required>
                </div>
                <div class="halfleft-textbox">
                <label for="nombre">Nombre</label>
                <input type="text" id="nombre" name="nombre" placeholder="Nombre" required>
                </div>    
                <div class="halfright-textbox">
                <label for="apellido">Apellido</label>
                <input type="text" id="apellido" name="apellido" placeholder="Apellido" required>
                </div>

                <label for="direccion">Dirección</label>
                <input type="text" id="direccion" name="direccion" placeholder="Dirección" required>

                <label for="ciudad">Ciudad</label>
                <input type="text" id="ciudad" name="ciudad" placeholder="Ciudad" required>

                <div id="estado-container">
                    <label for="estado">Estado</label>
                    <div class="styled-select blue semi-square" id="estado">
                        <select name="estado" required>
                            <option>Escoger</option>
                            <option>TX</option>
                            <option>...</option>
                        </select>
                    </div>
                </div>
                <div class="zip-code">
                    <label for="postal">Códigos Postal</label>
                    <input type="text" id="postal" name="postal" placeholder="">
                </div>
                <div id="business-type">
                    <div class="tipo-options">
                    <input type="radio" name="tipo" value="1" > 
                    <label for="tipo">Tipo de negocio 1</label>
                    </div>
                    <div class="tipo-options">
                    <input type="radio" name="tipo" value="2"> 
                    <label for="tipo">Tipo de negocio 2</label>
                    </div>
                    <div class="tipo-options">
                    <input type="radio" name="tipo" value="3"> 
                    <label for="tipo">Tipo de negocio 3</label>                    
                    </div>
                </div>
                <div class="align-left">
                    <input type="submit" value="Registrarse"> 
                </div>   
            </div>  
            <div class="align-right">
                <div class="modal-footer">
                    <input type="reset" id="clear-form" value="Cerrar">
                </div> 
            </div>
        </form>
    </div>
</section>

<script>
    var modal = document.getElementById('registrate-form');
    var tipo = document.getElementById('business-type');
    var title = document.getElementById('form-title');

    var indv = document.getElementById("indv");
    var fundacion = document.getElementById("fundacion");
    var agente = document.getElementById("agente");
    
    var span = document.getElementsByClassName("close")[0];
    var formType = document.getElementById("form_type");
    
    indv.onclick = function() {
      modal.style.display = "block";
      title.innerHTML = "Registro Individual";  
      tipo.style.display = "inline-block";
      form_type.value = 3;
    }

    fundacion.onclick = function() {
      modal.style.display = "block";
      title.innerHTML = "Registro Negocio o Fundacion";  
      tipo.style.display = "inline-block";
      form_type.value = 2;
    }

    agente.onclick = function() {
      modal.style.display = "block";
      title.innerHTML = "Registro de Agente LEAN";  
      tipo.style.display = "inline-block";
      form_type.value = 1;
    }
    
    window.onclick = function(event) {
      if (event.target == modal) {
        modal.style.display = "none";
      }
    }

    function validateForm(){
        var correo = document.forms["registration"]["correo"];
        var contrasena = document.forms["registration"]["contrasena"];
        var nombre = document.forms["registration"]["nombre"];
        var apellido = document.forms["registration"]["apellido"];
        var direccion = document.forms["registration"]["direccion"];
        var ciudad = document.forms["registration"]["ciudad"];
        var estado = document.forms["registration"]["estado"];
        var postal = document.forms["registration"]["postal"];
        if (correo.value == "")                              
        { 
            window.alert("This field is required."); 
            correo.focus(); 
            return false; 
        } 
        if (contrasena.value == "")                                  
        { 
            window.alert("This field is required."); 
            contrasena.focus(); 
            return false; 
        } 
        if (nombre.value == "" || !(/^[a-zA-Z ]*/.test(nombre)))                                  
        { 
            window.alert("This field is required."); 
            nombre.focus(); 
            return false; 
        } 
        if (apellido.value == "" || !(/^[a-zA-Z ]*/.test(apellido)))                                  
        { 
            window.alert("This field is required."); 
            apellido.focus(); 
            return false; 
        } 
        if (direccion.value == "")                                  
        { 
            window.alert("This field is required."); 
            correo.focus(); 
            return false; 
        } 
        if (ciudad.value == "")                                  
        { 
            window.alert("This field is required."); 
            ciudad.focus(); 
            return false; 
        } 
        if (estado.value == "")                                  
        { 
            window.alert("This field is required."); 
            estado.focus(); 
            return false; 
        } 
        if (postal.value == "")                                  
        { 
            window.alert("This field is required."); 
            postal.focus(); 
            return false; 
        } 
    }
</script>